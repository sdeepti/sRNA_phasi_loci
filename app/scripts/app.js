...
(function(window, $, _, undefined) {
    ...
    var showSearchError = function(json) {
        $('.progress', appContext).addClass('hidden');
        ...
        console.log(json);
        $('.error', appContext).html(errorMessage('The search returned an error: ' + json.obj.message));
    };
    ...
    var showSearchResult = function(json) {
        ...
        if (json.obj.status !== 'success') {
            ...
            $('.error', appContext).html(errorMessage('The search returned an error!'));
            return (false);
        }

        // These commands, in code or typed into the console, are useful for exploring the data.
        //console.log('JSON Object = ' + JSON.stringify(json.obj.result[0]));
        var loci = [];
        _.each(json.obj.result[0], function(value) {
            loci.push(value.locus_id);
        });
  ...
  window.addEventListener('Agave::ready', function() {
      ...
      '<div class="error"></div>' +
      '<div class="progress"><i class="fa fa-refresh fa-spin"></i> Searching...</div>' +
